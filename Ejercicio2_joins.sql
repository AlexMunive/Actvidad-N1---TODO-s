...
select accounts.account_no, accounts.client_id, account_types.name from account_types left join accounts on account_types.id=accounts.type;

--  Queries N°3

select transactions.id, transactions.amount,  transactions_types.name from transactions_types left join transactions on transactions_types.id= transactions.transaction_type where transactions.amount >= '500';

--  Queries N°4

select transactions.id, transactions.amount,  transactions_types.name from transactions_types left join transactions on transactions_types.id= transactions.transaction_type;
...
create view v_accounts_types as select accounts.account_no, accounts.client_id, account_types.name from account_types left join accounts on account_types.id=accounts.type;

create view v_transactions_detail as select transactions.id, transactions.amount,  transactions_types.name from transactions_types left join transactions on transactions_types.id= transactions.transaction_type where transactions.amount >= '500';

create view v_transations_types_detail as select transactions.id, transactions.amount,  transactions_types.name from transactions_types left join transactions on transactions_types.id= transactions.transaction_type;